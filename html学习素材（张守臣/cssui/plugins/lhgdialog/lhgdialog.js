...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	content: function( msg, add, frm )
	{
		...
		//ie8界面渲染延时问题导致界面宽高不正确，使用setTimeout增加延时 @editor 刘志波
		setTimeout(function(){
		// 新增内容后调整位置
		if( !add )
		{
...
$.dialog._tip = function( text, optss){
	var opts = optss;
    var api, aConfig, hide, wrap, top,
...
$.dialog.tip = function( text, time)
{
	var opts = {
    	    title: '提示',
    	    width: 240,  //必须指定一个像素宽度值或者百分比，否则浏览器窗口改变可能导致lhgDialog收缩
    	    height: 90,
    	    content: text,
    	    time: time ? time : 5
    	};
	 return $.dialog._tip( text, opts );
};